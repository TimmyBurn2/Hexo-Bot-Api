...
- **Scope:** this spec is the **server and ladder layer** only: identity, registration, matchmaking, challenges, ratings, and game lifecycle. The per-game move exchange is the **engine protocol**, reached through the session a `gameStart` event hands you; it is not in this repo.
